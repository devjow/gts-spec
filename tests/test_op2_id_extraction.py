...
class TestCaseTestOp2EntityRegistration_MissingId(HttpRunner):
    """OP#2 - Entity Registration: Instance must have id field"""
    config = Config("OP#2 - Entity Registration (missing id)").base_url(
        get_gts_base_url()
    )
    ...
    teststeps = [
        ...
        Step(
            ...
            .post("/entities")
            .with_json({
                "$$id": "gts://gts.x.test2.events.type.v1~",
                "$$schema": "http://json-schema.org/draft-07/schema#",
                "type": "object",
        ...
        Step(
            ...
            .with_json({
                "$$id": (
                    "gts.x.test2.events.type.v1~"
                    "x.test2.missing_id.event.v1.0~"
                ),
                "$$schema": "http://json-schema.org/draft-07/schema#",
                "type": "object",
                "allOf": [
                    {"$$ref": "gts://gts.x.test2.events.type.v1~"},
                    {
                        "type": "object",
        ...
        # Try to register instance without id field but with type field (should fail with 422)
        Step(
            RunRequest("register instance without id should fail")
            .post("/entities")
            .with_json({
                "type": (
                    "gts.x.test2.events.type.v1~"
